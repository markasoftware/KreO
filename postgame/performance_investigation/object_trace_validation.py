...
def loadTraces():
    ...
    for line in open(config["objectTracesPath"]):
        ...
        if len(line) == 1:
            ...
            curTrace.append(TraceEntry(line, methodStore.find_or_insert_method, 0))
    ...
    for line in open(config["objectTracesPath"] + "-name-map"):
        ...
        try:
            p1 = subprocess.Popen(
                ["demangle", "--noerror", "-n", splitlines[1]], stdout=subprocess.PIPE
            )
    ...
    for trace in traces:
        ...
        for entry in trace.traceEntries:
            if entry.isCall:
                ...
                assert (
                    len(entryStack) != 0
                ), f"object-trace invalid, call/returns don't match {trace}"
                assert (
                    entryStack[-1].method == entry.method
                ), f"object-trace invalid, call/returns don't match {trace}"
                entryStack.pop()
