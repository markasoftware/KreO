...
#ifndef KREO_CUSTOM_DATAFLOW_H
#define KREO_CUSTOM_DATAFLOW_H
...
public:
    ...
    VertexFlowGraphs buildGraphPerVertex(const CFG &cfg, size_t startVertex, VertexUnpacker vertexUnpacker) {
        ...
        mesg <<"; processed " <<Rose::StringUtility::plural(result.size(), "vertices", "vertex") <<"\n";
        return result;
    }
    ...
    public:
            ...
            if (!workList_.isEmpty()) {
                if (++nIterations_ > maxIterations_) {
                    throw NotConverging("data-flow max iterations reached"
                                        " (max=" + Rose::StringUtility::numberToString(maxIterations_) + ")");
                }
                    ...
                    if (!isFeasible_(cfg_, edge, state, incomingState_[nextVertexId])) {
                        ...
                        if (mlog[DEBUG]) {
                            ...
                            mlog[DEBUG] <<prefix() <<"    merge state is: "
                                        <<Rose::StringUtility::prefixLines(xfer_.toString(incomingState_[nextVertexId]),
                                                                     prefix() + "      ", false) <<"\n";
                        }
         ...
         * The worklist is processed from front to back.
          */
        void worklist(const WorkList &worklist) {
                workList_ = worklist;
        }
